...
fn build_show_website_params_uses_share_key_for_shared_entries() {
    ...
    let params = build_show_website_params(&account, &blob, &session, &vault_key).expect("params");
    ...
    assert!(
        crate::crypto::aes_decrypt_base64_lastpass(values["name"].as_str(), &vault_key).is_err()
    );
}
...
fn build_show_website_params_reports_missing_shared_folder_key() {
    ...
    let err =
        build_show_website_params(&account, &blob, &session, &key).expect_err("missing share key");
...
fn build_share_move_params_uses_target_share_key_for_encryption() {
    ...
    let params =
        build_share_move_params(&account, &blob, &session, &vault_key, Some("77")).expect("params");
    let values: std::collections::HashMap<_, _> = params.into_iter().collect();
    ...
    assert_eq!(String::from_utf8_lossy(&url), "https://example.com");
    assert!(
        crate::crypto::aes_decrypt_base64_lastpass(values["name0"].as_str(), &vault_key).is_err()
    );
}
...
fn build_share_move_params_uses_vault_key_when_moving_out_of_share() {
    ...
    let params =
        build_share_move_params(&account, &Blob::default(), &session, &vault_key, Some("77"))
            .expect("params");
    let values: std::collections::HashMap<_, _> = params.into_iter().collect();

    let name = crate::crypto::aes_decrypt_base64_lastpass(values["name0"].as_str(), &vault_key)
        .expect("decrypt name");
    assert_eq!(String::from_utf8_lossy(&name), "entry");
    assert_eq!(values["origsharedfolderid"], "77");
...
fn push_account_share_move_with_client_reports_http_and_api_failures() {
    ...
    let http_fail = HttpClient::mock_with_overrides(&[("lastpass/api.php", 500, "server error")]);
    ...
    let invalid = HttpClient::mock_with_overrides(&[("lastpass/api.php", 200, "<response/>")]);
    let err = push_account_share_move_with_client(
        &invalid,
        &session,
        &[3u8; KDF_HASH_LEN],
...
fn build_show_website_params_secure_note_uses_hex_url_and_field_payload() {
    let account = Account {
        ...
        password: Zeroizing::new(String::new()),
        password_encrypted: None,
        note: Zeroizing::new("extra".to_string()),
        note_encrypted: None,
        last_touch: String::new(),
        last_modified_gmt: String::new(),
        fav: false,
